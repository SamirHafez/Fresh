        ...
        public async Task UpdateAsync(ITraktService traktService)
        {
            if (Episodes.Count == 0)
            {
                ...
                foreach (var traktEpisode in await traktService.GetSeasonEpisodesAsync(TVShowId, Number, extended: TraktExtendEnum.MIN))
                {
                    ...
                    else if (traktEpisode.Updated_At != null && DateTime.Parse(traktEpisode.Updated_At, CultureInfo.InvariantCulture, DateTimeStyles.AdjustToUniversal) > episode.LastUpdated)
